...
const STYLES: Record<string, Record<string, string>> = {
  trigger: {
    container:
      'rounded-full flex items-center bg-[#111111] justify-center cursor-pointer outline-none ring-0 hover:brightness-125 transition-all duration-100 z-50',
    active: 'bg-[#111111]',
  },
  item: {
    container:
      'rounded-full flex items-center justify-center absolute bg-[#E8E0D5] hover:bg-[#E8E0D5]/70 cursor-pointer',
    label: 'text-xs text-[#111111] absolute top-full left-1/2 -translate-x-1/2 mt-1 whitespace-nowrap font-medium',
  },
}
...
const MenuTrigger = ({ setIsOpen, isOpen, itemsLength, closeAnimationCallback, openIcon, closeIcon }: MenuTriggerProps) => {
  ...
  const closeAnimation = async () => {
    shakeAnimation.start({
      ...
      transition: { duration: CONSTANTS.closeStagger, ease: 'linear', repeat: Infinity, repeatType: 'loop' as const },
    })
    ...
    <motion.div animate={shakeAnimation} className="z-50">
      ...
      >
        <AnimatePresence mode="popLayout">
          {isOpen ? (
            <motion.span key="menu-close" initial={{ opacity: 0, filter: 'blur(10px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }} exit={{ opacity: 0, filter: 'blur(10px)' }} transition={{ duration: 0.2 }}>
              {closeIcon}
            </motion.span>
          ) : (
            <motion.span key="menu-open" initial={{ opacity: 0, filter: 'blur(10px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }} exit={{ opacity: 0, filter: 'blur(10px)' }} transition={{ duration: 0.2 }}>
              {openIcon}
            </motion.span>
          )}
        </AnimatePresence>
...
const CircleMenu = ({
  items,
  openIcon = <Menu size={18} className="text-[#F5F0EB]" />,
  closeIcon = <X size={18} className="text-[#F5F0EB]" />,
}: {
  ...
  return (
    ...
    >
      <MenuTrigger
        ...
        closeIcon={closeIcon}
      />
      <motion.div animate={animate} className={cn('absolute inset-0 z-0 flex items-center justify-center')}>
        {items.map((item, index) => (
          <MenuItem key={`menu-item-${index}`} icon={item.icon} label={item.label} href={item.href} index={index} totalItems={items.length} isOpen={isOpen} />
        ))}
      </motion.div>
    </div>
  )
